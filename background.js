...
async function getAnswer(messages, profile_string) {
  var profile = JSON.parse(profile_string);
  var atempt = 0;
  const data = {
    messages: messages,
...
async function getPhoto(profile_string, username) {
  var profile = JSON.parse(profile_string);
  const data = {
    token: profile.token,
  ...
  try {
    ...
    if (responseData.success) {
      ...
      throw new Error('Запрос не выполнен успешно');
    }
  } catch (error) {
    console.error('Произошла ошибка:', error);
    throw error;
        ...
        try{
          if (should_respond(conversation.nickname,message.profile)){
            ...
            if (conversation.action == "send") {
              ...
              if (!checkIgnor) {
                var answers = await getAnswer(messages, message.profile)
                const answerArray = answers.split("\n");
                for (const answer of answerArray) {

                    if (answer == "!photo"){
                      var IMAGE_BASE64 = await getPhoto(message.profile, conversation.nickname)
